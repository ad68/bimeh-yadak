...
export default function Index({ open, hideModal, rowData, reloadTable }) {
  ...
  // ─── Life Cycle ─────────────────────────────────────────────────────────────────
  useEffect(() => {
    // console.log(contactUsTypeEnum.value);
  }, [contactUsTypeEnum]);
    ...
    <>
      <Modal width={531} open={open} onClose={hideModal}>
        <section className="flex flex-col">
          ...
          <Button loading={loading} onClick={edit} className="w-[150px] self-end h-[30px] text-xs xl:text-base xl:h-[40px] mt-8  z-10 bg-red rounded-[10px] text-white">
            ارسال
          </Button>
        </section>
      </Modal>
