  ...
  return (
    <section className="h-[400px]">
      ...
      <Checkbox.Group
        ...
        onChange={onChange}
      >
        <section className="grid grid-cols-2 xl:gap-x-8">
          <section className="m-2 flex items-center justify-start  rounded-lg border border-gray-100 p-2">
            <Checkbox value="withoutColoredPart" className="text-xs">
              بدون رنگ
            </Checkbox>
          </section>
          ...
          {coloredPartList.map((item, index) => (
            <section
              className="m-2 flex items-center justify-start rounded-lg border border-gray-100 p-2"
              key={index}
            >
              <Checkbox
                checked={true}
